...
get '/sc2/legacy/profile/:region_id/:realm_id/:profile_id/ladder' do |region_id, realm_id, profile_id|
  @options[:token] = TokenMap.instance.get(params[:token]) if params[:token]
  sc2_api_client.legacy(@region).ladders(region_id.to_sym, realm_id, profile_id, @options).to_json
end

get '/sc2/legacy/profile/:region_id/:realm_id/:profile_id/matches' do |region_id, realm_id, profile_id|
  @options[:token] = TokenMap.instance.get(params[:token]) if params[:token]
  sc2_api_client.legacy(@region).match(region_id.to_sym, realm_id, profile_id, @options).to_json
end
