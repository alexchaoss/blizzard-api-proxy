...
get '/hearthstone/cards' do
  ...
  hs_api_client.card(region: @region).search(search_options, **@options)&.to_json
end

get '/hearthstone/cards/:slug' do |slug|
  hs_api_client.card(region: @region).get(slug, 'constructed', **@options)&.to_json
end
...
get '/hearthstone/cardbacks' do
  ...
  hs_api_client.back(region: @region).search(search_options, **@options)&.to_json
end

get '/hearthstone/cards/:slug' do |slug|
  hs_api_client.back(region: @region).get(slug, **@options)&.to_json
end

# Deck

get '/hearthstone/deck/:slug' do |slug|
  hs_api_client.deck(region: @region).get(slug, **@options)&.to_json
end

# Metadata

get '/hearthstone/metadata' do
  hs_api_client.metadata(region: @region).index(**@options)&.to_json
end

get '/hearthstone/metadata/:type' do |type|
  hs_api_client.metadata(region: @region).get(type, **@options)&.to_json
end
